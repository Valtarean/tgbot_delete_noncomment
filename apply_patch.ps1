# apply_patch.ps1
# Safe Windows PowerShell script that creates repository files (UTF-8) and commits them.
# Usage (from repo root):
# 1) Open PowerShell as your user and cd to repository root.
# 2) Allow script execution for this session:
#      Set-ExecutionPolicy -ExecutionPolicy RemoteSigned -Scope Process -Force
# 3) Ensure you're on or tracking the branch:
#      git fetch origin
#      git checkout --track origin/improve/bot-pydantic-tests-ci
#    (or create it locally if tracking fails:
#      git checkout -b improve/bot-pydantic-tests-ci
#    )
# 4) Run: .\apply_patch.ps1
# The script will write files (overwriting if exist), git add, commit and push.
...
Write-Host "Starting apply_patch.ps1..."
...
'        notification = (',
'            "?? <b>Сообщение вне ветки обсуждения</b>\n\n"',
'            f"?? <b>Пользователь:</b> {name}"',
'        )',
'        if user and user.username:',
'            notification += f" (@{html.escape(user.username)})"',
'        safe_text = html.escape(text[:200]) if text else "?? Медиа без текста"',
'        notification += (',
'            f"\n?? <b>Текст:</b> {safe_text}"',
...
# docs/systemd/tgbot.service.example
$files['docs/systemd/tgbot.service.example'] = @(
'[Unit]',
...
# tests/test_warning_manager.py
$files['tests/test_warning_manager.py'] = @(
'import pytest',
'import asyncio',
'import os',
'from types import SimpleNamespace',
...
foreach ($path in $files.Keys) {
    ...
    Write-UTF8File -Path $path -Lines $files[$path]
}
...
git commit -m "Refactor: pydantic settings, split modules, add tests, CI, Dockerfile, README" --author="Copilot <copilot@example.com>"

Write-Host "Pushing to origin (current branch)..."
git push -u origin HEAD

Write-Host "Done. If push succeeded, create PR on GitHub (or use gh cli: gh pr create)."